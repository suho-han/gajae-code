...
- Telegram `/lean` notifications now mirror only the settled assistant answer when the agent goes idle, while `/verbose` keeps the previous per-tool-turn stream, reducing noisy mid-tool Telegram messages without hiding ask prompts.
